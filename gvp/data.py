        ...
        for line in tqdm.tqdm(dataset):
            if line !=None:
                try:
                    ...
                    entry['coords']=line['coords']
                    entry['seq']=line['seq']
                    assert torch.tensor(line['coords']).size()[1]!=1
                    assert torch.tensor(line['coords']).size()[0]>30
                    assert 'X' not in line['seq']
                    assert 'U' not in line['seq']
                    self.train.append(entry)
                except:
                    print(entry['name'])
        with open(test_path) as f:
            testdataset = json.load(f)
        for line in tqdm.tqdm(testdataset):
            if line !=None:
                try:
                    entry = {}
                    entry['name']=line['name']
...
class ProteinGraphDataset(data.Dataset):
    ...
    def __init__(self, data_list, 
        ...
        self.letter_to_num = {'C': 4, 'D': 3, 'S': 15, 'Q': 5, 'K': 11, 'I': 9,
                       ...
                       'N': 2, 'Y': 18, 'M': 12}
        self.pI = {'G':5.97,'A':6.00,'V':5.96,'L':5.98,'I':6.02,'P':6.30,'M':5.74,
                   'S':5.68,'C':5.07,'N':5.41,'Q':5.65,'T':5.60,'F':5.48,'Y':5.66,
                   'W':5.89,'D':2.97,'E':3.22,'R':10.76,'K':9.74,'H':7.59}
        self.polar = {'G':0,'A':0,'V':0,'P':0,'L':0,'I':0,'M':0,'F':0,'Y':1,'W':1,
                      'N':1,'Q':1,'C':1,'S':1,'T':1,'H':2,'K':2,'R':2,'D':3,'E':3}
    ...
    def _featurize_as_graph(self, protein):
        ...
        with torch.no_grad():
            ...
            # print('seq.len',seq.size())
            seqpI = torch.as_tensor([self.pI[a] for a in protein['seq']],device=self.device,dtype=torch.float32)
            # seqpolar = torch.as_tensor([self.polar[a] for a in protein['seq']],device=self.device,dtype=torch.long)
            # print('seqpi.len',seqpI.size())
            
            mask = torch.isfinite(coords.sum(dim=(1,2)))
            coords[~mask] = np.inf
            X_ca = coords[:, 1]
            edge_index = torch_cluster.knn_graph(X_ca, k=self.top_k)                                             #此处会根据knn紧邻来计算topk个紧邻，形式代表edge的index,为每个点找最近的30个点
            ...
            # print(seqpI.size())
            # node_s = dihedrals
            node_s = torch.cat([dihedrals,seqpI.unsqueeze(-1)],dim=1)
            # assert edge_index.size()[1] == X_ca.size()[0]*30,name+':'+str(edge_index.size()[1])+'!='+str(X_ca.size()[0]*30)+'not equal.'
            # node_s = torch.cat([dihedrals,seqpI[edge_index[0]].view(-1,self.top_k)],dim=1)
            
            # print(name)
            # print('orientations.size',orientations.size())
            # print('concat.size',seqpI[edge_index[0]].view(-1,self.top_k).size())
            # print('pos_embeddings',pos_embeddings.size())
            # print('rbf',rbf.size())
        ...
        # embedding = torch.nn.Embedding(4,8,padding_idx=0)
        # node_polar = embedding(seqpolar)
        # node_s = torch.cat([node_s,node_polar],dim=1)
    ...
    def _positional_embeddings(self, edge_index, 
                               num_embeddings=None,
                               period_range=[2, 1000]):                                             #Tansformer当中，使用位置编码。位置的信息如何高效建模，trans中
        # From https://github.com/jingraham/neurips19-graph-protein-design                          #认为在POS i 位置和 i+k位置，应该在某个维度之间是有相关性的。
