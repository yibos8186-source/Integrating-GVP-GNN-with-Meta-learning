    ...
    def sample(self, h_V, edge_index, h_E, n_samples, temperature=0.1):
        ...
        with torch.no_grad():
            ...
            for i in range(L):
                ...
                # print(i,torch.argmax(logits).cpu().numpy(),F.softmax(logits)[0,torch.argmax(logits)].cpu().numpy())
                seq[i::L] = Categorical(logits=logits / temperature).sample()
                # print(i,seq[i].cpu().numpy(),F.softmax(logits)[0,seq[i]].cpu().numpy())
                value_dict[i]=F.softmax(logits)[0,:].cpu().numpy()
                h_S[i::L] = self.W_s(seq[i::L])
                
            return seq.view(n_samples, L),value_dict
